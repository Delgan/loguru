    ...
    def __init__(self,
        *,
        ...
        super(JSONHandler, self).__init__(
            ...
            id_=id_,
            levels_ansi_codes=levels_ansi_codes)

        self.requiry_key = self.parse()

    def parse(self):
        import re
        standard_formatters = re.compile(r'{(.+?)[:}]', re.IGNORECASE)
        return standard_formatters.findall(self._formatter)
    ...
    def emit(self, record, level_id, from_decorator, is_ansi, is_raw):
        ...
        try:
            ...
            if self._filter is not None:
                if not self._filter(record):
                    return
            self.requiry_key = list(set(record.keys()).intersection(set(self.requiry_key)))
            tmp_dict = {k: record[k] for k in self.requiry_key}
            if 'time' in tmp_dict:
                tmp_dict['time'] = tmp_dict['time'].strftime('%Y-%m-%d %H:%M:%S.%f')
            if 'level' in tmp_dict:
                tmp_dict['level'] = tmp_dict['level'].name
