...
def test_time_rotation(monkeypatch_date, tmpdir, when, hours):
    ...
    monkeypatch_date(now.year, now.month, now.day, now.hour, now.minute, now.second, now.microsecond)
...
def test_time_rotation_dst(monkeypatch_date, tmpdir):
    ...
    i = logger.start(tmpdir.join("test_{time}.log").realpath(), format='{message}', rotation="1 day")
    logger.debug("First")
...
@pytest.mark.parametrize('rotation', [
    ...
    "w5 at [not|a|time]", "[not|a|day] at 12:00",
    object(), os, datetime.date(2017, 11, 11), datetime.datetime.now(), 1j,
])
